      ...
      <p>• +{job.requirements.length - 3} more…</p>
        ...
        {job.postedDate && (
          <div className="flex justify-end mt-4 pt-3 border-t border-amber-200/60">
            <span className="text-sm text-black font-medium">📅 Posted {job.postedDate}</span>
          </div>
        )}
      </li>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md">
          <div className="bg-gradient-to-br from-amber-50 via-yellow-50 to-orange-50 w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl p-8 shadow-2xl animate-fadeIn border-2 border-amber-200/80">
            <div className="flex justify-between items-start mb-6">
              <h3 className="text-3xl font-bold text-black tracking-tight">{job.title}</h3>
              <button 
                onClick={closeDetail}
            ...
            <div className="space-y-6">
              ...
              {job.salary && (
                <div className="mb-6">
                  <h4 className="text-xl font-bold text-black mb-2">💰 Salary Range</h4>
              ...
              {job.description && (
                <div className="bg-gradient-to-r from-amber-50/80 to-yellow-50/80 rounded-xl p-4 border border-amber-200/60">
                  <h4 className="text-xl font-bold text-black mb-3">📝 Description</h4>
                  <p className="text-base text-black leading-relaxed font-medium">{job.description}</p>
                </div>
              )}

              {/* Key Responsibilities */}
              <div className="mb-6">
                <h4 className="text-xl font-bold text-black mb-3">🎯 Key Responsibilities</h4>
                <ul className="list-disc list-inside space-y-2 text-black font-medium">
                  <li>Manage construction projects from planning to completion</li>
                  ...
                  <li>Monitor project timelines and budgets</li>
                  <li>Supervise construction teams and subcontractors</li>
                </ul>
              </div>

              {/* Required Skills & Qualifications */}
              <div className="bg-gradient-to-r from-yellow-50/80 to-orange-50/80 rounded-xl p-4 border border-amber-200/60">
                <h4 className="text-xl font-bold text-black mb-3">🔧 Required Skills & Qualifications</h4>
                {job.requirements && job.requirements.length > 0 ? (
                  ...
                  <ul className="list-disc list-inside space-y-2 text-black font-medium">
                    <li>Bachelor's degree in Construction Management or related field</li>
              ...
              <div className="border-t-2 border-amber-200/60 pt-6 bg-gradient-to-r from-amber-50/80 to-yellow-50/80 rounded-xl p-4">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input type="text" className="w-full border-2 border-amber-200/60 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-amber-400 bg-white/90 text-black font-medium" />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-black mb-2">Email:</label>
                    <input type="email" className="w-full border-2 border-amber-200/60 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-amber-400 bg-white/90 text-black font-medium" />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-black mb-2">Phone:</label>
                    <input type="tel" className="w-full border-2 border-amber-200/60 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-amber-400 bg-white/90 text-black font-medium" />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-black mb-2">Cover Letter:</label>
                    <textarea rows={4} className="w-full border-2 border-amber-200/60 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-amber-400 bg-white/90 text-black font-medium" placeholder="Tell us why you're interested in this position..."></textarea>
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-black mb-2">Resume/CV:</label>
                    <input type="file" className="w-full border-2 border-amber-200/60 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-amber-400 bg-white/90 text-black font-medium" accept=".pdf,.doc,.docx" />
                  </div>
                  <button 
                    onClick={() => {
                      alert('Application submitted!');
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md">
          <div className="bg-gradient-to-br from-amber-50 via-yellow-50 to-orange-50 w-full max-w-md rounded-2xl p-8 shadow-2xl animate-fadeIn border-2 border-amber-200/80">
            <h3 className="text-2xl font-bold mb-6 text-black">Apply for {job.title}</h3>
            <form onSubmit={handleSubmit} className="space-y-4">
              <input type="text" name="name" placeholder="Name" required className="w-full rounded-lg border-2 border-amber-200/60 px-4 py-3 focus:border-amber-400 focus:ring-amber-400 bg-white/90 text-black font-medium" />
              <input type="email" name="email" placeholder="Email" required className="w-full rounded-lg border-2 border-amber-200/60 px-4 py-3 focus:border-amber-400 focus:ring-amber-400 bg-white/90 text-black font-medium" />
              <input type="tel" name="phone" placeholder="Phone (optional)" className="w-full rounded-lg border-2 border-amber-200/60 px-4 py-3 focus:border-amber-400 focus:ring-amber-400 bg-white/90 text-black font-medium" />
              <textarea name="message" rows={4} placeholder="Cover letter / message" className="w-full rounded-lg border-2 border-amber-200/60 px-4 py-3 focus:border-amber-400 focus:ring-amber-400 bg-white/90 text-black font-medium" />
              <div className="flex justify-end gap-3 pt-2">
                <button type="button" onClick={close} className="px-6 py-3 rounded-lg bg-gradient-to-r from-gray-200 to-amber-100 text-black hover:from-gray-300 hover:to-amber-200 font-semibold transition-all duration-300">Cancel</button>
                <button type="submit" className="px-6 py-3 rounded-lg bg-gradient-to-r from-amber-600 to-orange-600 text-white hover:from-amber-700 hover:to-orange-700 font-bold transition-all duration-300 shadow-lg hover:shadow-xl">Submit</button>
              </div>
            </form>
          </div>
        </div>
